...
// Funcion que muestra en el DOM que no hay coincidencias de busqueda
function noMatches() {
  const noMatches = d.createElement("DIV");
  noMatches.classList.add("alert", "error");
  noMatches.textContent = "No hay coincidencias";
  resultado.appendChild(noMatches);
}
...
// Función que filtra en base al tipo de transmisión del auto
function filterTrans(auto) {
  const { transmision } = searchObj;
  if (transmision) return auto.transmision === transmision;
  return auto;
}
// Función que filtra en base al tipo de color del auto
function filterColor(auto) {
  const { color } = searchObj;
